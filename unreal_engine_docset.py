...
def collector_default(
    ...
    """
    Collect the *API* data for given elements.
...
def collector_nohref(
    elements: List[lxml.html.HtmlElement],
    api_information: ApiInformation,
    html_path: Path,
) -> List[Tuple[str, str]]:
    """
    Collect the *API* data for given elements without an ``href``.
...
COLLECTORS_CPP: Tuple[Collector, ...] = (
    Collector(
        "Module",
        './/div[@class="modules-list"]//td[@class="name-cell"]/a',
        collector_default,
    ),
    Collector(
        "Class",
    ...
    Collector(
        "Enum",
        './/div[@id="enums"]//td[@class="name-cell"][1]/a',
        collector_default,
    ),
    Collector(
        "Variable",
...
"""*Unreal Engine* documentation processors."""
    ...
    for entry in sorted(entries, key=lambda x: x.name):
        cursor.execute(
            ...
            (
                entry.name,
                entry.type,
                entry.path.replace(f"{documents_directory}/", ""),
            ),
        )
...
def generate_docset(input: str, output: str) -> None:
    ...
    if docset_type == "blueprint":
        ...
        label = "Blueprint"
        processor = lambda x: set()  # noqa: E731
    ...
    with contextlib.chdir(api_directory):  # pyright: ignore
